...
- `interaction gaze-live` or `python scripts/phase7_live_webcam_smoke.py` for the live webcam gaze path
...
- The current tester-facing MVP path now includes live voice, live webcam gaze, and a live sequential multimodal `fusion-live` demo. Webcam gaze remains heuristic and region-based, and the multimodal path is still sequential rather than a continuous concurrent session stack.
